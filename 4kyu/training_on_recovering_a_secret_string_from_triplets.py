...
# In particular, this means that the secret string will never contain letters that do not occur in
...
def recoverSecret(triplets):
    temp = []
    result = []
    for indx, values in enumerate(triplets):
        for letters in values:
            if letters not in temp:
                temp.append(letters)
